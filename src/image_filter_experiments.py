        ...
        for window in windows_per_noise[noise_type]:
            for i in range(batch_count):
                ...
                for i in range(batch_size):
                    seed = get_new_seed()
                    p = Process(target=evolve_binary_classifier, args=(classifiers_seeds_per_noise[noise_type], noise_type, window, seed,))
                    p.start()
                    runs.append(p)
                for p in runs:
                    p.join()

evolve_image_filters(batch_size=2, batch_count=1 ,noises=noise_types)
